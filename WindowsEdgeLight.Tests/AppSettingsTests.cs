...
public sealed class AppSettingsTests : IDisposable
{
    ...
    [Fact]
    public void EmptyObjectReturnsDefaults()
    {
        Directory.CreateDirectory(tempDirectory);
        File.WriteAllText(SettingsPath, "{}");
    ...
    [Fact]
    public void PartialObjectPreservesDefaults()
    {
        Directory.CreateDirectory(tempDirectory);
    ...
    [Fact]
    public void OutOfRangeValuesAreNormalized()
    {
        Directory.CreateDirectory(tempDirectory);
        ...
        Assert.Equal(1.0, settings.Brightness);
        Assert.Equal(0.0, settings.ColorTemperature);
    }

    [Fact]
    public void SaveFailureDoesNotThrow()
    {
        var directoryAsFilePath = Path.Combine(tempDirectory, "some-dir");
        Directory.CreateDirectory(directoryAsFilePath);

        var exception = Record.Exception(() => new AppSettings().SaveTo(directoryAsFilePath));
