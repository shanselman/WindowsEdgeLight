...
🔥 Warmer Color Temp:  Ctrl + Shift + ←
❄️ Cooler Color Temp:  Ctrl + Shift + →
...
Version {version}";
    ...
    private void RegisterGlobalHotKeys(IntPtr hwnd)
    {
        ...
        if (!RegisterHotKey(hwnd, HOTKEY_COLOR_TEMP_WARMER, MOD_CONTROL | MOD_SHIFT, VK_LEFT))
        {
            failedHotKeys.Add("Warmer Color Temp (Ctrl+Shift+Left)");
        }

        if (!RegisterHotKey(hwnd, HOTKEY_COLOR_TEMP_COOLER, MOD_CONTROL | MOD_SHIFT, VK_RIGHT))
        {
            failedHotKeys.Add("Cooler Color Temp (Ctrl+Shift+Right)");
        }
