...
🔥 Warmer Colour:  Ctrl + Shift + →
❄️ Cooler Colour:  Ctrl + Shift + ←
...
Version {version}";
    ...
    private void RegisterGlobalHotKeys(IntPtr hwnd)
    {
        ...
        if (!RegisterHotKey(hwnd, HOTKEY_COLOR_TEMP_WARMER, MOD_CONTROL | MOD_SHIFT, VK_RIGHT))
        {
            failedHotKeys.Add("Warmer Colour Temp (Ctrl+Shift+Right)");
        }

        if (!RegisterHotKey(hwnd, HOTKEY_COLOR_TEMP_COOLER, MOD_CONTROL | MOD_SHIFT, VK_LEFT))
        {
            failedHotKeys.Add("Cooler Colour Temp (Ctrl+Shift+Left)");
        }
